...
-   HTTP는 무상태 프로토콜이기 때문에 클라이언트와 서버가 요청과 응답을 주고 받으면 연결이 끊어진다.
-   클라이언트가 다시 요청을 보내면 서버는 이전 요청을 기억하지 못한다.
-   → 쿠키 사용

#### Set-Cookie

-   서버에서 클라이언트로 쿠키 전달(응답)
-   클라이언트에서는 응답 받은 쿠키를 쿠키 저장소에 저장

#### Cookie

-   쿠키 저장소에서 조회해서, HTTP 요청 시 서버로 전달

#### 쿠키

-   사용 예시: 사용자 로그인 세션 관리, 광고 정보 트래킹
-   쿠키 정보는 항상 서버로 전송됨
    -   네크워크 트래픽 추가 유발
    -   최소한의 정보만 사용(세션 id, 인증 토큰)
    -   서버에 전송하지 않고, 웹 브라우저 내부에 데이터를 저장하기 위해서는 웹 스토리지(localStorage, sessionStorage) 사용
-   보안에 민감한 데이터는 저장하면 안 됨

#### 쿠키 생명주기(Expires, max-age)

-   Set-Cookie: expires=Mon, 12-Feb-2024 21:06:36 GMT
    -   만료일이 되면 쿠키 삭제
...
-   명시: 명시한 문서 기준 도메인 + 서브 도메인 포함
    -   domain=example.com을 지정해서 쿠키 생성
        -   example.com과 dev.example.com 모두 쿠키 접근 가능
-   생략: 현재 문서 기준 도메인만 적용
    -   example.com에서 쿠키를 생성하고 domain 지정을 생략
        -   example.com에서만 쿠키 접근 가능
        -   dev.example.com은 쿠키 접근 불가능
